...
export default function TestimonialsCarousel({
  ...
  return (
    <section className={cn("relative py-24 bg-background border-t border-border overflow-hidden transition-colors duration-300", className)}>
      {/* Animated Background Elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-primary/10 rounded-full blur-[128px] -translate-y-1/2"></div>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-primary/10 rounded-full blur-[128px] translate-y-1/2"></div>
        
        {/* Moving dot grid */}
        <motion.div 
          className="absolute inset-0 z-0 bg-[radial-gradient(#8882_1px,transparent_1px)] [background-size:32px_32px] opacity-20"
          animate={{
          ...
          <div className="flex gap-6 lg:gap-8 px-3">
            {duplicatedTestimonials.map((testimonial, index) => (
              <div
                ...
                className="flex flex-col justify-between rounded-2xl border border-border bg-card p-8 shadow-sm transition-all hover:shadow-md hover:border-border/80 shrink-0 w-[320px] md:w-[400px]"
              >
                <div className="mb-8">
                  ...
                  <p className="text-foreground/95 text-base leading-relaxed">
                    "{testimonial.text}"
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4 mt-auto pt-6 border-t border-border/50">
                  <Avatar className="h-10 w-10 rounded-full border border-border">
                    <AvatarImage src={testimonial.imageSrc} alt={testimonial.name} loading="lazy" decoding="async" />
                    <AvatarFallback className="bg-secondary text-foreground">{testimonial.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div>
